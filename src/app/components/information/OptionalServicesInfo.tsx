...
const OptionalServicesInfo: React.FC<OptionalServicesInfoProps> = ({
  locale,
}) => {
    ...
    <div className="w-full">
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {Object.values(localeData.OPTIONAL_TESTS).map((service, index) => (
          <div
            key={index}
            className="relative rounded-lg shadow-lg bg-white pb-12"
          >
            <div
            ...
            >
              <h3 className="text-white font-bold text-lg md:text-2xl">
                {service.TITLE.replace(/[a-zA-Z]\d- /g, "")}
              </h3>
            </div>
            <div className="p-2">
              <p className="text-xs md:text-sm lg:text-base mb-4 p-2">
                {service.DESCRIPTION}
              </p>
            </div>
            <div className="py-4">
            ...
            <div className="py-4">
              <p className="text-xs md:text-sm lg:text-base">
                {localeData.BASIC.REQUIRED_TIME}: {service.TIME}
              </p>
              <div className="w-full flex flex-row items-center justify-center">
                <p className="text-xs md:text-sm lg:text-base">
                  {localeData.BASIC.TYPE}: {service.TYPE}
                </p>
                <div
            ...
            <Link
              style={{ backgroundColor: getColorCode(service.TYPE) }}
              className={`absolute bottom-2 right-2 text-white px-4 py-2 rounded-md`}
