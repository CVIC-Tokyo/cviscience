  ...
  return (
    <div className=" bg-white h-auto rounded-lg shadow-md pb-8 relative">
      ...
      <div className="p-4">
        ...
        {price ? (
          typeof price === "string" ? (
            <p className="font-semibold text-sm md:text-base lg:text-lg mb-2">
              {price}
            </p>
          ) : (
            <div className="text-sm md:text-base lg:text-lg mb-2">
              {Object.entries(price).map(([key, value], index) => (
                <p key={index}>
                  {" "}
                  <span className="font-semibold text-sm md:text-base lg:text-lg mb-2">
                    {value}
                  </span>
                </p>
              ))}
        ...
        <p className="mb-2 text-[10px] md:text-xs italic text-cvic-red">
          {recommendation ? recommendation : null}
        </p>
      </div>
      <Link
        className="absolute bottom-2 right-2 bg-cvic-red text-white px-4 py-2 rounded-md"
